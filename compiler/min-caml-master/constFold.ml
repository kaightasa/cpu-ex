...
let rec g env = function (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | LShift(x, y) when memi x env && memi y env -> Int((findi x env) lsl (findi y env)) (* 掛け算（シフト）のケース (挙動不審) *)
  | RShift(x, y) when memi x env && memi y env -> Int((findi x env) lsr (findi y env))
